...
| 작업 | 예상 시간 | 비고 |
|------|----------|------|
| Supabase Storage 버킷 공개 설정 | 10분 | 대시보드만 클릭 |
| Supabase RLS 정책 적용 | 20분 | SECURITY.md SQL 복붙 |
| 실기기 빌드 + 통합 테스트 | 1시간 | 카카오·FCM·Storage 확인 |
| 시연 영상 촬영 (30초) | 1시간 | 아이폰 화면 녹화 |
| 발표 리허설 (7분) | 30분 | |
...
- [ ] **실기기 통합 테스트**
  ...
  - [v] 카카오 닉네임 정상 저장 확인 ("다 울" → "다울")
  - [ ] 카카오 프로필 사진 정상 로드 확인 (getPhotoPath 수정 후 미확인)
  - [v] 회원가입 선택 사진 프로필에서 정상 표시 확인 (동일 수정)
  - [v] 핀 생성 → Supabase 대시보드에서 저장 확인
  - [v] 로그아웃 → 재로그인 → 핀 자동 복원 확인
...
- [ ] 발표용 QR코드 생성 (앱 다운로드 or 랜딩페이지 링크) -> 발표 시작할때 혹시라도 제 발표 듣기 지루하신분들은 이 큐알 찍어서 제가 그래도 뭘 했는지 간략하게 보실 수 있습니다 .;
