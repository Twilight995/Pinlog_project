...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  Future<void> _drawRoute(List<PinModel> pins) async {
    ...
    final (lineColorInt, markerBg, markerText, markerBorder, haloColorInt) = switch (style) {
      MapStyleOption.satellite => (
        const Color(0xFFFFFFFF).toARGB32(),
        Colors.white,
        const Color(0xFF1C1C1E),
        const Color(0xFF1C1C1E),
        const Color(0xFFB0C4DE).toARGB32(),
      ),
      MapStyleOption.dark => (
    ...
    // 폴리라인 연결 (glow + halo + main 3겹)
    if (pins.length >= 2) {
      final geometry = LineString(
        coordinates: pins.map((p) => Position(p.longitude, p.latitude)).toList(),
      );
